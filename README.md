...
1. Provide user with basic instructions on how to interact with the UI, and allow user to enter a city name and country code. The user must also enter an API key
2. When the user clicks on the Submit button, the result is displayed below
   ...
   2. If there have been validation issues that were non-fatal, a warning will also be displayed in addition to the weather description if possible.
   3. If the valiation issues were fatal, an error message will be displayed to inform the user how to proceed.
...
2. This endpoint requires the following:
   1. A query parameter specifying city
   2. Another query parameter specifying the country code
   3. A valid API key
3. There are 5 API keys that have been created.
...
`curl --location 'http://localhost:5000/weather?country=au&city=melbourne' \
--header 'API-KEY: d096e3523c9e439397870d6528588a5b'`
...
## When a valid city and country code have been entered
...
Note: if there were additional weather descriptions, they would also be comma separated within quotation marks
...
## When an invalid city and invalid country code is provided
